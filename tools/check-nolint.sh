#!/usr/bin/env bash
# tools/check-nolint.sh -- Validate nolint annotation format (HS#8)
...
for file in "$@"; do
  ...
  if grep -nP '# nolint\s*$' "$file"; then
    echo "BLOCKED (HS#8): $file — bare '# nolint' without linter name and reason" >&2
    errors=1
  fi

  # Blanket # nolint start/end
  if grep -nP '# nolint\s+(start|end)' "$file"; then
    echo "BLOCKED (HS#8): $file — blanket '# nolint start/end' is prohibited" >&2
    errors=1
  fi

  # # nolint: with no linter (just colon, nothing after)
  if grep -nP '# nolint:\s*$' "$file"; then
    echo "BLOCKED (HS#8): $file — '# nolint:' requires linter name and reason" >&2
    errors=1
  fi

  # # nolint: <linter> without ". <reason>"
  if grep -nP '# nolint:\s+\w+\s*$' "$file"; then
    echo "BLOCKED (HS#8): $file — '# nolint: <linter>' requires a reason: '# nolint: <linter>. <reason>'" >&2
    errors=1
  fi

  # # nolint: <linter>. (period but empty/whitespace-only reason)
  if grep -nP '# nolint:\s+\w+\.\s*$' "$file"; then
    echo "BLOCKED (HS#8): $file — '# nolint: <linter>.' requires a non-empty reason after the period" >&2
    errors=1
  fi
done
