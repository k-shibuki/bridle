...
When an S7 class property references another S7 class defined in a different file, the source file MUST have an `@include` roxygen directive to guarantee load order. Without it, the class definition may fail at load time depending on file collation order.
