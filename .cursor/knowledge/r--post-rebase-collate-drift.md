...
**Policy** (defined in `@.cursor/rules/quality-policy.mdc` § Post-Rebase Collate Drift): After every rebase with new R files, run `roxygen2::roxygenise()`, commit the updated DESCRIPTION/NAMESPACE, then proceed with `make format` and `make gate-fast`.
