...
- Run quality checks on `main` (`make check`)
...
- If `lintr` shows issues, try `lintr --fix`
...
make style          # Format auto-fix (styler)
...
- [ ] Lint/type checks pass (`make check`)
...
1. **No warnings remain** from `lintr` / `R CMD check`
